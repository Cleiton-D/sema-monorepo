...
import ClassroomClassReportItem from 'components/ClassroomClassReportItem';
...
const ClassroomClassReportTemplate = () => {
  ...
  const schoolTermsMap = useMemo(() => {
    ...
    return schoolTermPeriods
      ...
      .reduce<Array<{ name: string; schoolTermPeriod?: SchoolTermPeriod }>>(
        ...
        [{ name: 'Anual', schoolTermPeriod: undefined }]
      );
  }, [schoolTermPeriods]);
  ...
  const tabItems = useMemo(() => {
    return schoolTermsMap.map(({ name, schoolTermPeriod }) => {
      return {
        title: name,
        element: (
          <ClassroomClassReportItem schoolTermPeriod={schoolTermPeriod} />
        )
      };
    });
  }, [schoolTermsMap]);
  ...
  return (
    <Base>
      <Heading>Relatório de aulas - {classroom?.description}</Heading>
...
export default ClassroomClassReportTemplate;
