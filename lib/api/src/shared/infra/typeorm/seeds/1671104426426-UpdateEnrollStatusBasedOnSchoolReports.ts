...
export default class UpdateEnrollStatusBasedOnSchoolReports1671104426426
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    const schoolReports: SchoolReport[] = await queryRunner.query(`
      SELECT school_report.id as id,
       ...
       WHERE school_report.first IS NOT NULL
         ...
         AND (
              school_report.first_rec IS NOT NULL
              OR
              school_report.second_rec IS NOT NULL
            )
         AND school_report.exam IS NULL

         AND (
              school_report.status = 'RECOVERY'
              OR
              school_report.status = 'ACTIVE'
              OR
              school_report.status = 'EXAM'
            )
    `);
    ...
    const promises = schoolReports.map(schoolReport => {
      const result = schoolReport.final_average >= 600 ? 'APPROVED' : 'EXAM';
